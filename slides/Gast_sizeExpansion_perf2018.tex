...
\begin{frame}{Mean Field and Refined Mean Field Approximations}
  ...
  \begin{tabular}{c|c|c|c|c|c}
    ...
    % &\uncover<2->{Simu. ($N{=}10$)}&\uncover<3->{Refined Mean
    %                                  Field}\\
...
\begin{frame}{ Question addressed in this work}
  % \begin{align*}
  %   Perf(N) \approx Perf(\infty)+\frac{V}{N}
  % \end{align*}
  ...
  \end{align*}
  %Provide differential equation and analyze the cost. 
\end{frame}
...
\begin{frame}{Density dependent population process (Kurtz, 70s)}
  %{where $O(1)$ objects change states at the same time}
  
  % $X$ denotes the empirical measure.
  % \begin{align*}
  %   X_i(t) = \text{ fraction of objects in state $i$ }
  % \end{align*}
  % \bigskip\pause
...
\begin{frame}{Expected values estimated by mean field are
  ...
  % \blue{Theorem} (Kolokoltsov 2012, G. 2017\& 2018). If the drift $f$
  % is $C^2$ and has a unique exponentially stable attractor, then for
  % any $t\in[0,\infty)\cup\{\infty\}$, there exists a constant $V_t$
  % such that:
  % \begin{align*}
  %   \displaystyle \esp{h(X^N(t))}= h(x(t)) + \frac{V(t)}{N} + O(1/N^2)
  % \end{align*}
\end{frame}
  ...
  \begin{align*}
    \dt \esp{X_i^2} &= \esp{(2X_i+\frac1{N})(X_{i-1}^2-X_i^2)
                    ...
                    &=\esp{
                      ...
                      ?}}} + \dots\dots\dots\dots}
                     % &=\esp{2X(t) -
                     %   \underbrace{2X(t)^3}_{\green{\esp{X^3}\approx?}}
                     %   - 2X(t)^2 + 
                     %   \frac1N(1+X(t)-X(t)^2)}
  \end{align*}
...
% \begin{frame}{System Size Expansion Approach}
%   Using this approach, there exists a system of ordinary

%   {\tiny 
%     \begin{align*}
%       \esp{(X-x)}&=\frac1NV(t)+\frac1{N^2}A(t)+O(1/N^3)\\
%       \esp{(X-x)^{\otimes2}}&=\frac1NW(t)+\frac1{N^2}B(t)+O(1/N^3)\\
%       \esp{(X-x)^{\otimes3}}&=\frac1{N^2}C(t)+O(1/N^3)\\
%       \esp{(X-x)^{\otimes4}}&=\frac1{N^2}D(t)+O(1/N^3)\\
%       \esp{(X-x)^{\otimes5}}&=O(1/N^3)
%     \end{align*}
%   }
%   Let $x(t)$ be the mean field approximation and $Y(t)=X(t)-x(t)$.

%   Main idea to estimate $\esp{X}$.
%   \begin{itemize}
%   \item
%     $\dt Y(t)^{(k)}=\underbrace{Y(t)\otimes\dots\otimes
%       Y(t)}_{\text{$k$ times}}$ can be expressed as (an exact)
%     function of others $Y(t)^{(j)}$.
%   \end{itemize}
%   \pause
  
%   \begin{exampleblock}{}
%     You can close the equations by assuming that $Y^{(k)}=0$ for $k\ge
%     K$.  
%     \begin{itemize}
%     \item For $K=1$, this gives the mean field approximation
%       ($1/N$-accurate)
%     \item For $K=3$, this gives the refined mean field
%       ($1/N^2$-accurate). 
%     \item For $K=5$, this gives a second order expansion
%       ($1/N^3$-accurate).
%     \end{itemize}
%   \end{exampleblock}
%   Limit of the approach: For a system of dimension $d$, $Y(t)^{(k)}$
%   has $d^k$ equations.
% \end{frame}
